...
int main(void) {
    ...
    api->set_param(inst, "scale", "0.25");
    require_true(get_float(api, inst, "scale") < 0.26f, "set_param moves the param");
    ...
    require_true(strcmp(name, "Passthrough") == 0, "preset 0 name is Passthrough");
    require_true(get_float(api, inst, "scale") > 1.0f - 0.001f
                     && get_float(api, inst, "scale") < 1.0f + 0.001f,
    ...
    api->set_param(inst, "scale", "0.25");
    api->set_param(inst, "preset", "0");
    require_true(get_float(api, inst, "scale") < 0.26f, "re-selecting the same preset does not reapply it");
