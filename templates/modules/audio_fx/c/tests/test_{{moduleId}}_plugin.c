...
int main(void) {
    ...
    require_true(get_float(api, inst, "mix") < 0.26f, "set_param moves the param");
    ...
    require_true(get_float(api, inst, "mix") > 0.5f - 0.001f
                     && get_float(api, inst, "mix") < 0.5f + 0.001f,
    ...
    api->set_param(inst, "mix", "0.25");
    api->set_param(inst, "preset", "0");
    require_true(get_float(api, inst, "mix") < 0.26f, "re-selecting the same preset does not reapply it");
