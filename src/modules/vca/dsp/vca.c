...
static void* create_instance(const char *module_dir, const char *config_json) {
    (void)module_dir;
    (void)config_json;
    vca_plugin_t *p = (vca_plugin_t*)calloc(1, sizeof(vca_plugin_t));
    if (p) {
        vca_init(&p->core);
        /* Deliberately no preset at create: vca_init has applied
        ...
        p->current_preset = -1;
        mf_scope_init(&p->scope, VCA_SCOPE_WINDOW, VCA_SCOPE_MODE, VCA_SCOPE_STYLE);
    }
    return p;
}
...
static void process_block(void *instance, int16_t *audio_inout, int frames) {
    vca_plugin_t *p = (vca_plugin_t*)instance;
    ...
    vca_process_float(&p->core, p->in_l, p->in_r, p->out_l, p->out_r, frames);
...
static void set_param(void *instance, const char *key, const char *val) {
    vca_plugin_t *p = (vca_plugin_t*)instance;
    if (!p || !key || !val) return;
    /* The Rack panics a lane through this key rather than the note wire, so a
     * gate that a note-off never reached still closes. */
    if (strcmp(key, "all_notes_off") == 0) {
        vca_all_notes_off(&p->core);
        return;
    }
    if (strcmp(key, "preset") == 0) {
        ...
        int next = vca_clamp_preset_index(atoi(val));
        if (next == p->current_preset) return;
        p->current_preset = next;
        vca_apply_preset(&p->core, p->current_preset);
        return;
    }
    vca_set_param(&p->core, vca_param_id(key), (float)atof(val));
}
...
static int get_param(void *instance, const char *key, char *buf, int buf_len) {
    ...
    if (strcmp(key, "preset_count") == 0) {
        return snprintf(buf, (size_t)buf_len, "%d", vca_preset_count());
    }
    if (strcmp(key, "preset") == 0) {
        return snprintf(buf, (size_t)buf_len, "%d", p->current_preset);
    }
    if (strcmp(key, "preset_name") == 0) {
        return snprintf(buf, (size_t)buf_len, "%s", vca_preset_name(p->current_preset));
    }
    if (strcmp(key, "__scope") == 0) {
        return mf_scope_serialize(&p->scope, buf, buf_len);
    }
    int id = vca_param_id(key);
    if (id < 0) return -1;
    return snprintf(buf, (size_t)buf_len, "%.6f", vca_get_param(&p->core, id));
}
...
static void on_midi(void *instance, const uint8_t *msg, int len, int source) {
    ...
    vca_handle_midi(&p->core, msg[0], msg[1], msg[2]);
}
