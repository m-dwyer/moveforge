...
static void* create_instance(const char *module_dir, const char *json_defaults) {
    (void)module_dir;
    (void)json_defaults;
    swarf_plugin_t *p = (swarf_plugin_t*)calloc(1, sizeof(swarf_plugin_t));
    if (!p) return NULL;
    swarf_init(&p->core);
    /* Deliberately no preset at create: swarf_init has applied
    ...
    p->current_preset = -1;
    mf_scope_init(&p->scope, SWARF_SCOPE_WINDOW, SWARF_SCOPE_MODE, SWARF_SCOPE_STYLE);
    return p;
}
...
static void on_midi(void *instance, const uint8_t *msg, int len, int source) {
    (void)source;
    swarf_plugin_t *p = (swarf_plugin_t*)instance;
    if (!p || len < 3) return;
    uint8_t status = msg[0] & 0xF0;
    if (status == 0x90 && msg[2] > 0) {
        swarf_note_on(&p->core, msg[1], (float)msg[2] / 127.0f);
        mf_scope_arm(&p->scope);  /* restart the scope frame on note onset */
    } else if (status == 0x80 || (status == 0x90 && msg[2] == 0)) {
        swarf_note_off(&p->core, msg[1]);
    } else if (status == 0xE0) {
        swarf_pitch_bend(&p->core, moveforge_midi_bend_normalized(msg[1], msg[2]));
    }
}
...
static void set_param(void *instance, const char *key, const char *val) {
    ...
    if (strcmp(key, "preset") == 0) {
        ...
        int next = swarf_clamp_preset_index(atoi(val));
        if (next == p->current_preset) return;
        p->current_preset = next;
        swarf_all_notes_off(&p->core);
        swarf_apply_preset(&p->core, p->current_preset);
        return;
    }
    swarf_set_param(&p->core, swarf_param_id(key), (float)atof(val));
}
...
static int get_param(void *instance, const char *key, char *buf, int buf_len) {
    ...
    if (strcmp(key, "preset_count") == 0) {
        return snprintf(buf, (size_t)buf_len, "%d", swarf_preset_count());
    }
    if (strcmp(key, "preset") == 0) {
        return snprintf(buf, (size_t)buf_len, "%d", p->current_preset);
    }
    if (strcmp(key, "preset_name") == 0) {
        return snprintf(buf, (size_t)buf_len, "%s", swarf_preset_name(p->current_preset));
    }
    if (strcmp(key, "__scope") == 0) {
        return mf_scope_serialize(&p->scope, buf, buf_len);
    }
    int param_id = swarf_param_id(key);
    if (param_id < 0) return -1;
    return snprintf(buf, (size_t)buf_len, "%.6f", swarf_get_param(&p->core, param_id));
}
...
static void render_block(void *instance, int16_t *out, int frames) {
    ...
    swarf_process_float(&p->core, NULL, NULL, left, right, frames);
...
plugin_api_v2_t* move_plugin_init_v2(const host_api_v1_t *host) {
    g_host = host;
    if (g_host && g_host->log) g_host->log("[swarf] init v2");
    return &g_api;
}
