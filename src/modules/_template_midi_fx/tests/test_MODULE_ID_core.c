...
int main(void) {
    ...
    int scale_id = MODULE_ID_param_id("scale");
    require_true(scale_id >= 0, "param lookup works");
    require_true(MODULE_ID_param_id("does_not_exist") < 0, "unknown param lookup fails");

    /* Clamp test */
    MODULE_ID_set_param(&fx, scale_id, 10.0f);
    require_true(MODULE_ID_get_param(&fx, scale_id) <= 2.0f, "scale clamps high");

    /* Scale down: vel 100 * 0.5 -> 50 */
    MODULE_ID_set_param(&fx, scale_id, 0.5f);
    uint8_t in_note_on[3] = { 0x90, 60, 100 };
    uint8_t out_msgs[4][3];
    int out_lens[4];
    int emitted = MODULE_ID_process_midi(&fx, in_note_on, 3, out_msgs, out_lens, 4);
    ...
    emitted = MODULE_ID_process_midi(&fx, in_note_off, 3, out_msgs, out_lens, 4);
    ...
    MODULE_ID_set_param(&fx, scale_id, 0.0f);
    uint8_t in_quiet[3] = { 0x90, 60, 1 };
    MODULE_ID_process_midi(&fx, in_quiet, 3, out_msgs, out_lens, 4);
    require_true(out_msgs[0][2] >= 1, "velocity clamps to 1 minimum");

    /* Tick emits nothing in this passthrough template */
    int ticked = MODULE_ID_tick(&fx, 128, 44100, out_msgs, out_lens, 4);
