...
| `lobber` | Lobber | C | Tempo-locked slice buffer with three play modes — Live (toss/stutter/reverse/throw the live buffer), Loop (capture a bar and play it tempo-followed), Slice (fire slices of the captured loop over the live input). Played via pads (toss grid on ch0, function row on ch1) or knobs. Syncs to host tempo, falls back to a bpm param; Loop mutes on transport stop. Params: active, offset, division, mode, loop, ratchet, reverse, freeze, mute, capture, loop_beats, mix, bpm, xfade. Parity roadmap: [`src/modules/lobber/PARITY.md`](src/modules/lobber/PARITY.md). |
