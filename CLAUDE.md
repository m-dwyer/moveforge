...
- `tools/render_fx.c` is the offline host harness for audio FX. It generates a test signal (sweep/noise/impulse/silence) or reads an input WAV, streams it through `move_audio_fx_init_v2` in 128-frame blocks, and writes the processed WAV. MIDI FX modules do not have an offline harness yet.
