...
- `src/modules/<module-id>/dsp/<module-id>_params.gen.h` and `<module-id>_params.gen.inc` are both **generated** from `module.json` by `scripts/gen-params.ts`. **Do not edit either by hand** — re-run `mise run gen-params` after editing `module.json`.
  - The `.h` holds only `<UPPER>_PARAM_COUNT` and the `<UPPER>_PARAM_*` enum, and is included by `<module-id>_core.h`. It exists because the core header needs the param count to size arrays indexed by param id (a Faust adapter's `zones[]`), and the `.inc` cannot serve that purpose: it defines functions taking `<module-id>_core_t *`, so it has to come *after* the struct. Nothing else may `#define` or `#undef` `<UPPER>_PARAM_COUNT`; `validate-params` rejects a core header that does.
  - The `.inc` defines `<module>_param_id`, `<module>_set_param` (with clamps from min/max), `<module>_get_param`, and `<module>_apply_defaults`, and for Faust modules a `_Static_assert` that `zones[]` matches the param count. Included from `<module>_core.c` (plain C) or `<module>_adapter.c` (Faust); wrappers and tests use the public functions (string keys), not the enum.
  - For plain C, the state struct must have one `float` field per param key; for Faust, the adapter holds them as fields plus captured Faust zone pointers.
...
7. Update `capabilities.ui_hierarchy.levels.root.knobs` if the param should be on the Move encoders. Entries are grouped into banks of 8 in the generated chain UI; the first 8 are the parent Schwung screen's main encoder mapping.
8. Run `mise run gen-ui-chain` if the chain-mode parameter surface changed.
9. Focused tests in `tests/test_<module-id>_core.c`.
10. Run `mise run validate` (re-checks gen-params, gen-faust, gen-presets, and gen-ui-chain drift).
