...
for MODULE_ID in $MODULE_IDS; do
  ...
  if needs_rebuild "$WASM_OUT" "${WASM_DEPS[@]}"; then
    COMMANDS+=("emcc '$MODULE_DIR/dsp/${MODULE_ID}_wasm.c' '$MODULE_DIR/dsp/${MODULE_ID}_core.c' -O3 -I'$MODULE_DIR/dsp' -s STANDALONE_WASM=1 -s EXPORTED_FUNCTIONS='[\"_mf_init\",\"_mf_set_param\",\"_mf_note_on\",\"_mf_note_off\",\"_mf_all_notes_off\",\"_mf_set_pitch_bend\",\"_mf_left_ptr\",\"_mf_right_ptr\",\"_mf_render\"]' -Wl,--no-entry -o '$WASM_OUT'")
