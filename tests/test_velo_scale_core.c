...
int main(void) {
    velo_scale_core_t fx;
    velo_scale_init(&fx);

    int scale_id = velo_scale_param_id("scale");
    require_true(scale_id >= 0, "param lookup works");
    require_true(velo_scale_param_id("does_not_exist") < 0, "unknown param lookup fails");

    /* Clamp test */
    velo_scale_set_param(&fx, scale_id, 10.0f);
    require_true(velo_scale_get_param(&fx, scale_id) <= 2.0f, "scale clamps high");

    /* Scale down: vel 100 * 0.5 -> 50 */
    velo_scale_set_param(&fx, scale_id, 0.5f);
    uint8_t in_note_on[3] = { 0x90, 60, 100 };
    uint8_t out_msgs[4][3];
    int out_lens[4];
    int emitted = velo_scale_process_midi(&fx, in_note_on, 3, out_msgs, out_lens, 4);
    ...
    emitted = velo_scale_process_midi(&fx, in_note_off, 3, out_msgs, out_lens, 4);
    ...
    velo_scale_set_param(&fx, scale_id, 0.0f);
    uint8_t in_quiet[3] = { 0x90, 60, 1 };
    velo_scale_process_midi(&fx, in_quiet, 3, out_msgs, out_lens, 4);
    require_true(out_msgs[0][2] >= 1, "velocity clamps to 1 minimum");

    /* Tick emits nothing in this passthrough template */
    int ticked = velo_scale_tick(&fx, 128, 44100, out_msgs, out_lens, 4);
    require_true(ticked == 0, "tick emits nothing");

    printf("velo_scale core tests passed\n");
    return 0;
}
